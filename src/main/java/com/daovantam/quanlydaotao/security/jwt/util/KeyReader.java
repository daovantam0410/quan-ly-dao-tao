...
public final class KeyReader {
    ...
    public static PrivateKey getPrivateKey(String privateKey) throws NoSuchAlgorithmException, InvalidKeySpecException, IOException {
        ...
        PKCS8EncodedKeySpec pkcs8EncodedKeySpec = new PKCS8EncodedKeySpec(pemObject.getContent()); // mã hóa pemObject
        KeyFactory keyFactory = KeyFactory.getInstance("RSA"); // mã hóa theo thuật toán RSA
        PrivateKey key = keyFactory.generatePrivate(pkcs8EncodedKeySpec); //generate thành privateKey
